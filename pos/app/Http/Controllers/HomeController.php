...
class HomeController extends Controller
{
    ...
    public function index()
    {
        ...
        if(Auth::user()->role_id > 2 && $general_setting->staff_access == 'own') {
            $PosRevenue = Sale::whereDate('created_at', '>=' , $start_date)->where('user_id', Auth::id())->whereDate('created_at', '<=' , $end_date)->sum('grand_total'); //POS
            $WebRevenue = DB::table('orders')->whereDate('created_at', '>=' , $start_date)->where('user_id', Auth::id())->whereDate('created_at', '<=' , $end_date)->where('status','Confirmed')->sum('amount');
            ...
            $recent_sale_web = DB::table('orders')->where('status','Confirmed')->where('user_id', Auth::id())->orderBy('id', 'desc')->take(5)->get();
            ...
            $PosRevenue = Sale::whereDate('created_at', '>=' , $start_date)->whereDate('created_at', '<=' , $end_date)->sum('grand_total');
            $WebRevenue = DB::table('orders')->whereDate('created_at', '>=' , $start_date)->whereDate('created_at', '<=' , $end_date)->where('status','Confirmed')->sum('amount');
            $revenue = $PosRevenue + $WebRevenue;
            ...
            $recent_sale_web = DB::table('orders')->where('status','Confirmed')->orderBy('id', 'desc')->take(5)->get();
       ...
       $best_selling_qty_web = OrderDetails::select(DB::raw('product_id, sum(count) as sold_qty'))->whereDate('created_at', '>=' , $start_date)->whereDate('created_at', '<=' , $end_date)->groupBy('product_id')->orderBy('count', 'desc')->take(5)->get();

       $yearly_best_selling_qty_web = OrderDetails::select(DB::raw('product_id, sum(count) as sold_qty'))->whereDate('created_at', '>=' , date("Y").'-01-01')->whereDate('created_at', '<=' , date("Y").'-12-31')->groupBy('product_id')->orderBy('count', 'desc')->take(5)->get();

       $yearly_best_selling_price_web = OrderDetails::select(DB::raw('product_id, sum(amount) as total_price'))->whereDate('created_at', '>=' , date("Y").'-01-01')->whereDate('created_at', '<=' , date("Y").'-12-31')->groupBy('product_id')->orderBy('total_price', 'desc')->take(5)->get();
        ...
        while($start < $end)
        {
            ...
            if(Auth::user()->role_id > 2 && $general_setting->staff_access == 'own') {
                ...
                $WebRecievedAmount = DB::table('orders')->whereDate('created_at', '>=' , $start_date)->whereDate('created_at', '<=' , $end_date)->where('user_id', Auth::id())->where('status','Confirmed')->sum('amount');
                $recieved_amount =  $recieved_amount + $WebRecievedAmount;
                ...
                $WebRecievedAmount = DB::table('orders')->whereDate('created_at', '>=' , $start_date)->whereDate('created_at', '<=' , $end_date)->where('status','Confirmed')->sum('amount');
                $recieved_amount =  $recieved_amount + $WebRecievedAmount;
        ...
        while($start < $end)
        {
            ...
            if(Auth::user()->role_id > 2 && $general_setting->staff_access == 'own') {
                ...
                $WebRecievedAmount = DB::table('orders')->whereDate('created_at', '>=' , $start_date)->whereDate('created_at', '<=' , $end_date)->where('user_id', Auth::id())->where('status','Confirmed')->sum('amount');
                $sale_amount = $sale_amount + $WebRecievedAmount;
        ...
        //return $month;
        return view('index', compact('revenue', 'purchase', 'expense', 'return', 'purchase_return', 'profit', 'payment_recieved', 'payment_sent', 'month', 'yearly_sale_amount', 'yearly_purchase_amount', 'recent_sale','recent_sale_web','recent_purchase', 'recent_quotation', 'recent_payment', 'best_selling_qty', 'yearly_best_selling_qty', 'yearly_best_selling_price','best_selling_qty_web', 'yearly_best_selling_qty_web', 'yearly_best_selling_price_web'));
    }
    ...
    public function dashboardFilter($start_date, $end_date)
    {
        $general_setting = DB::table('general_settings')->latest()->first();
        if(Auth::user()->role_id > 2 && $general_setting->staff_access == 'own') {
            $PosRevenue = Sale::whereDate('created_at', '>=' , $start_date)->whereDate('created_at', '<=' , $end_date)->where('user_id', Auth::id())->sum('grand_total');
            $WebRevenue = DB::table('orders')->whereDate('created_at', '>=' , $start_date)->whereDate('created_at', '<=' , $end_date)->where('status','Confirmed')->sum('amount');
            $revenue = $PosRevenue + $WebRevenue;
