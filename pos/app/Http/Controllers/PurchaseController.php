...
class PurchaseController extends Controller
{
    ...
    public function productWithVariant()
    {
        return Product::join('product_variants', 'products.id', 'product_variants.product_id')
                ...
                ->select('products.id', 'products.name', 'product_variants.item_code', 'product_variants.variant_name')
    ...
    public function productPurchaseData($id)
    {
        ...
        foreach ($lims_product_purchase_data as $key => $product_purchase_data) {
            ...
            if($product_purchase_data->variant_id) {
                ...
                $product->code = isset($lims_product_variant_data->item_code) ? $lims_product_variant_data->item_code : '';
            }
    ...
    public function edit($id){
        ...
        if($role->hasPermissionTo('purchases-edit')){
            ...
            $lims_product_purchase_data = ProductPurchase::where('purchase_id', $id)->get();
           if(count($lims_product_purchase_data) > 0){
            ...
            return redirect()->back()->with('not_permitted', 'Sorry! No data found.');
           }
    ...
    public function addPayment(Request $request){
        ...
        $account = DB::table('accounts')->where('is_active', 1)->get();
        if(count($account) < 1 ){ 
            return redirect('purchases')->with('not_permitted', 'Please add an account first.');
            exit;
        }

        $account = DB::table('accounts')->where('id', $data['account_id'])->where('is_active', 1)->first();
        if(!$account){ 
            return redirect('purchases')->with('not_permitted', 'Please add a valid account first.');
            exit;
        }
