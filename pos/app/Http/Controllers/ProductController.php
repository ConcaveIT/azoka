...
class ProductController extends Controller
{
    ...
    public function store(Request $request){
       ...
       $this->validate($request, [
        ...
        //Product Slug
        if($request->slug){
            $allSlugs = $this->getRelatedSlugs($request->slug, 0);
            if (! $allSlugs->contains('slug', $request->slug)){
                $data['slug'] = $request->slug;
            }else{
                $data['slug'] = $request->slug.'-1';
            }
...
//        $data['expiry_date'] = $request->expiry_date;
        ...
        if(isset($data['is_variant'])) {
            ...
            foreach ($data['variant_name'] as $key => $variant_name) {
                ...
                $lims_product_variant_data->variant_name = $data['variant_name'][$key];
                $lims_product_variant_data->variant_by = $data['variant_by'][$key];
    ...
    public function edit($id){
        ...
        if ($role->hasPermissionTo('products-edit')) {
            ...
            // var_dump($lims_product_variant_data); exit;
            $variant_data = DB::table('product_variants')->where('product_id', $id)->get();
            //return var_dump($lims_product_variant_data);
            return view('product.edit',compact('lims_product_list', 'lims_brand_list', 'lims_category_list', 'lims_unit_list', 'lims_tax_list', 'lims_product_data', 'lims_product_variant_data', 'variant_data'));
        }
        else
            return redirect()->back()->with('not_permitted', 'Sorry! You are not allowed to access this module');
    }
    ...
    public function updateProduct(Request $request){
        if(!env('USER_VERIFIED')) {
            ...
            $data['name'] = htmlspecialchars($data['name']);
            //Product Slug
            $have = DB::table('products')->where('slug', $request->slug)->first();
            if($have){
                $id_by_slug = $have->id;
            }else{
                $id_by_slug = 0;
            }
            if($request->id != $id_by_slug){
                if($request->slug){
                    $allSlugs = $this->getRelatedSlugs($request->slug, 0);
                    if (! $allSlugs->contains('slug', $request->slug)){
                        $data['slug'] = $request->slug;
                ...
                foreach ($data['variant_name'] as $key => $variant_name) {
                    if($data['product_variant_id'][$key] == 0) {
                        ...
                        ProductVariant::find($data['product_variant_id'][$key])->update([
                            'position' => $key+1,
                            'variant_name' => $data['variant_name'][$key],
   ...
   public function variant_delete($variant_id){
    $data = DB::table('product_variants')->where('id', $variant_id)->first();
    if($data){
        DB::table('product_variants')->where('id', $variant_id)->delete();
        return 1;
    }else{
        return 0;
    }
